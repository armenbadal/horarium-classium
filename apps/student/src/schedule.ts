...
export const scheduleUrl =
  'https://gist.githubusercontent.com/armenbadal/d384e13e190a1d6ff9fb9a1f89205ae0/raw/horarium-classium.json';
  ...
  for (const [dayName, lessons] of Object.entries(remoteSchedule)) {
    ...
    for (let index = 1; index < sorted.length; index++) {
      if (sorted[index].start < sorted[index - 1].end) {
        throw new Error(
          `Overlapping lessons for ${dayName}: ${sorted[index - 1].start}–${sorted[index - 1].end} and ${sorted[index].start}–${sorted[index].end}.`
        );
      }
    }
