...
- Supabase-ի հրապարակված դասացուցակին միացում՝ դասարանի UUID կոդով, առանց Student հաշվի։
...
Teacher-ում ընտրեք դասարանը, պահպանեք սևագիրը, սեղմեք «Հրապարակել», ապա «Պատճենել դասարանի կոդը»։ Student-ի առաջին մեկնարկը բացում է «Միանալ դասարանին» ձևը։ Կոդը ստուգելուց հետո երևում են դպրոցի և դասարանի անունները. «Միանալ»-ը պահպանում է ընտրությունը։ «Փոխել դասարանը» նույն ընթացքով փոխարինում է այն միայն հաջող պահպանումից հետո։ Կոդը հանրային `publicId` UUID-ն է, ոչ Teacher-ի ներքին class ID-ն։

`src/publication.ts`-ը կանչում է `get_published_schedule` RPC-ն և ստուգում envelope-ի formatVersion/publicId/revision/publishedAt/անունները/IANA timezone-ը, ապա առանձին՝ `schedule` դաշտը։ Network/HTTP/JSON/validation սխալի դեպքում օգտագործվում է միայն նույն Supabase URL-ին և դասարանին պատկանող կրկին ստուգված cache-ը։ Առանց դրա երևում են սխալն ու «Կրկին փորձել»-ը։ Դասարանի ստուգման կամ պահման ձախողումը չի փոխում նախկին հաստատված կապը։
...
- `publication-v2.json`՝ միջավայրը, ընտրված `publicId`-ը և ամբողջ հրապարակումը կամ `publication: null` անվավերացումը,
- `publication-v2.invalidated.json`՝ անվավերացման լրացուցիչ նշիչ. պաշտպանում է նաև հին cache ֆայլի փոխարինման ձախողման դեպքում,
